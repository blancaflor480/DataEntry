...
import '../style/addrecord.css'; // Add this new CSS file
...
const AddRecordModal = ({ show, onHide, employees, onRecordAdded }) => {
  const [formData, setFormData] = useState({
    ...
    incident_time: new Date().toTimeString().split(' ')[0].substr(0, 5),
    department:'',
    location: '',
    description: '',
    witnesses: '',
...
const deptHeadSearchRef = useRef(null);
  ...
  useEffect(() => {
    if (searchTerm && !formData.employee_no) { // Only filter if no employee is selected
  ...
  // Add this new handler for department head selection
  ...
  const handleChange = (e) => {
    ...
    setFormData(prev => ({
      ...prev,
      [name]: value,
      // Reset incident_type when incident_category changes
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const employee = employees.find(emp => emp.employeeNo === formData.employee_no);
      if (!employee) {
      ...
      await axios.post('http://localhost:5000/incident-reports', formDataToSend);
  ...
  return (
    <Modal show={show} onHide={onHide} size="lg">
      <Modal.Header closeButton>
        <Modal.Title>Create Incident Report</Modal.Title>
      </Modal.Header>
      <Modal.Body>
            ...
            <div className="col-md-6">
              ...
              <Form.Group className="mb-3">
                ...
                >
                  ...
                  <option value="Sales Department">Sales Departmen</option>
                </Form.Select>
              </Form.Group>
